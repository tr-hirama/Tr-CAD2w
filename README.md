...
- **画面**: 配色は [Tr-viewer](https://github.com/tr-hirama/Tr-viewer)（DXF / TRC ビューワー）に合わせた**ダーク 1 トーン**。**図面の背景は別**で、「背景」ボタンで白 / 暗を切り替え
...
- **概要・注記文・境界コメント・メモ**: 現場名 / 概要コード / 作業者 / 備考、デスクトップ版と同じ**標準注記文 40 件**からの選択と本文の編集、境界番号と境界標の種類、メモ。**概要コード（無ければ現場名）が保存名の既定**になります。**手書きメモは Web でも描けます**（ツールバーの「手書き」）。点列（`MemoStrokes`）で持ち、ペン・消しゴム・4 色。**Windows Ink の ISF（`MemoInk`）は持ちません**（issue #39・案B）
- **まわりけん（境界辺長）**: `.tc2` の `Ken` / `Keisanten` を読み、サイドバーに一覧で出します（**入力欄は作りません**）。**誤差（|まわりけん − 計算値|）を色で示します** — 20mm 以下は水色、50mm 以下は金、超えると `Over`（赤）。非数値は `－`、計算値なしは `No point`、測れない辺は `不可`。入れ直しが要る辺の数も出します。**周長は境界が `K1` からの連番で揃っているときだけ**出し、欠番があれば理由を添えて出しません（欠けた辺のぶん短い値が出て、正しく見えてしまうため）
- **レベル（水準）**: `.tc2` の `Level` を読み、**器高式**（器械高 = 後視 + 地盤高、地盤高 = 器械高 − 前視）で解いてサイドバーに一覧で出します（**表示だけ**）。転換点（`TP`）は前視として読み、**確定した地盤高は初回のみ**（既知点への点検で上書きしません）。解けない行は数と理由を出します
...
| 向き | 落ちるもの |
|---|---|
| **`.tc2` → Web** | 測量データ（観測 / 座標 / 座標変換）、用紙空間、グループ |
| **Web → `.tc2`** | 画層の**線種**（デスクトップ版の画層は色と表示だけ）、用紙空間 |

寸法・ハッチ・ブロック（`Insert`＋ブロック定義）・画像・まわりけんは**双方向で往復します**。
...
**ツールバーは今使えるボタンだけを出します。** 押しても「先に○○してください」と断られる
ボタン（寸法を選んでいないときの **寸法値**、レベルが入っていないときの **帳票Excel**、
モデル空間での **ビューポート**、履歴が空のときの **元に戻す** など）は表示されません。
条件が揃うと現れます。**キー操作はボタンの表示に関係なく、いつでも効きます**（`Del` `Ctrl+Z` など）。
...
| `src/core/ink.ts` | 手書きメモの点列。間引き（Ramer–Douglas–Peucker）と消しゴム |
| `src/core/comments.ts` | 標準注記文 40 件（デスクトップ版の `コメント一覧.csv` を写したもの） |
| `src/survey/ken.ts` | まわりけん（境界辺長）。連番の検査と周長。**表示だけで値は作らない** |
| `src/survey/level.ts` | レベル（水準）の器高式。**確定 GH は初回のみ**（点検誤差を伝播させない） |
...
| `src/ui/toolbar.ts` | ツールバーのボタンを今出すかの判定（**押しても断られるボタンは出さない**） |
