...
- **座標変換**: `.tc2` の `Transform`（共通点）を読み、**ヘルマート（相似・4パラメータ）とアフィン（6パラメータ）**を最小二乗で解いてサイドバーに出します（**表示だけ**）。倍率・回転・移動と**残差（最大 / RMS）を必ず併記**します（最小二乗は共通点が悪くても解を返すので、残差を見ないと合っていない変換に気づけないため）
...
| 向き | 落ちるもの |
|---|---|
| **`.tc2` → Web** | 測量データ（観測 / 座標 / まわりけん / レベル）、用紙空間、グループ |
...
| `src/survey/transform.ts` | 座標変換（ヘルマート／アフィン）。最小二乗と残差。**部分ピボットのガウス消去** |
