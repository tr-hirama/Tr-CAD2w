...
/**
 * 誤差(mm)を出す。**mm の整数へ丸めてから返す**（デスクトップ版に合わせる）。
 *
 * デスクトップ版 `MawarikenRow.ConfirmGosa`（`SurveyInput.cs`）は
 *
 * ```csharp
 * int gosaMm = (int)Math.Round(Math.Abs(m - _calcDist) * 1000.0, MidpointRounding.AwayFromZero);
 * ```
 *
 * と **mm の整数**にしてからしきい値と比べている。ここも同じにする。理由は 2 つ。
 *
 * 1. **判定を向こうと一致させる。** μm まで見ると、誤差 20.4mm が Web では金・
 *    デスクトップ版では水色（20 に丸まる）と分かれてしまう
 * 2. **浮動小数のゴミが確実に消える。** `10.050 - 10` は `50.00000000000071`、
 *    `123456.050 - 123456.000` は `50.00000000291038` になる。**このズレは座標が
 *    大きいほど育つ**ので固定の許容値では吸収しきれないが、mm 整数へ丸めれば
 *    座標の桁によらず**境界値は必ず等号側に入る**
 *
 * C# の `AwayFromZero` と JS の `Math.round` は負の 0.5 の扱いが違うが、ここは
 * **絶対値なので必ず非負**。0.5 は両方とも 1 へ上がるので差は出ない。
 */
function gosaMm(measured: number, calcDist: number): number {
  return Math.round(Math.abs(measured - calcDist) * 1000);
}
